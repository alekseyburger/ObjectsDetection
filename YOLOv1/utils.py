...
from model_output import CELLS_PER_DIM, CLASSES_NUM, BOXES_NUM, BOXES_AREA_LEN, OUTPUT_LEN
...
def intersection_over_union(prediction_box, target_box, hw_image_proportional = False):
    ...
    Parameters:
        prediction_box (tensor): Predictions of Bounding Boxes (BATCH_SIZE, 4)
...
def convert_cellboxes(predictions, S=CELLS_PER_DIM):
    """
    ...
    box1_confidence = moutput_box_confidence(predictions,0)[...,0]
    box2_confidence = moutput_box_confidence(predictions,1)[...,0]
    scores = torch.cat(
        (box1_confidence.unsqueeze(0),
        box2_confidence.unsqueeze(0)),
        dim=0)
